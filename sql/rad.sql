...
the UW_rpt.vEGC1 table, so we include an alternate query that pulls from
UW.vEGC1 instead.
*/
-- Original query using UW_rpt.vEGC1
WITH
	-- Add row numbers to deduplicate vMODs by cost center.
	...
	-- Extract base RAD data by joining to the deduplicated MOD
	base_data AS (
		SELECT
			...
			egc1.FECDMSponsorName,
			egc1.FECDMSponsorEntityType,
			egc1.FECDMOriginatingSponsorName,
			egc1.FECDMOriginatingSponsorEntityType,
			...
			awrd.AwardScheduleContractEndDate,
		...
		WHERE
			mod.row_num = 1 -- Deduplicate by filtering to first row
	)
-- Filter to MOD of interest
SELECT
	*
FROM
	base_data
